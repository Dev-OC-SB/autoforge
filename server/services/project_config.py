...
Configuration is stored in {project_dir}/.seaforge/config.json.
"""
...
def _get_config_path(project_dir: Path) -> Path:
    ...
    Checks the new .seaforge/ location first, falls back to .autocoder/
    ...
    """
    new_path = project_dir / ".seaforge" / "config.json"
    if new_path.exists():
        return new_path
...
def _save_config(project_dir: Path, config: dict) -> None:
    ...
    Creates the .seaforge directory if it doesn't exist.
    ...
    # Ensure the .seaforge directory exists
...
def clear_dev_command(project_dir: Path) -> None:
    ...
    if not config:
        try:
            ...
            # Also remove .seaforge directory if empty
            seaforge_dir = config_path.parent
            if seaforge_dir.exists() and not any(seaforge_dir.iterdir()):
                seaforge_dir.rmdir()
                logger.debug("Removed empty .seaforge directory for %s", project_dir.name)
        except OSError as e:
