 ...
 * =============
 *
 * Main CLI module for the SeaForge npm global package.
...
function ensureEnvFile() {
  ...
  if (existsSync(ENV_EXAMPLE)) {
    ...
    writeFileSync(ENV_FILE, '# SeaForge configuration\n# See documentation for available options.\n', 'utf8');
  }
  return true;
}
...
function startServer(opts) {
  ...
  if (existingPid && isProcessAlive(existingPid)) {
    log(`SeaForge is already running at http://${host}:${port}`);
    log('Opening browser...');
  ...
  if (host !== '127.0.0.1') {
    ...
    console.log('  The SeaForge UI will be accessible from other machines.');
