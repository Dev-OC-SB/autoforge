...
def test_blocked_command_via_hook():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        seaforge_dir.mkdir()
        (seaforge_dir / "allowed_commands.yaml").write_text(
            "version: 1\ncommands: []"
        )
...
def test_project_config_allows_command():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        seaforge_dir = project_dir / ".seaforge"
        seaforge_dir.mkdir()
        (seaforge_dir / "allowed_commands.yaml").write_text("""version: 1
commands:
  - name: swift
...
def test_invalid_yaml_ignored():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        seaforge_dir = project_dir / ".seaforge"
        seaforge_dir.mkdir()
        (seaforge_dir / "allowed_commands.yaml").write_text("invalid: yaml: content:")
