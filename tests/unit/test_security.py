...
def test_yaml_loading():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        seaforge_dir = project_dir / ".seaforge"
        seaforge_dir.mkdir()

        # Test 1: Valid YAML
        config_path = seaforge_dir / "allowed_commands.yaml"
...
""")
        ...
        (project_dir / ".seaforge" / "allowed_commands.yaml").unlink()
        config = load_project_commands(project_dir)
        if config is None:
...
""")
                ...
                project_seaforge = project_dir / ".seaforge"
                project_seaforge.mkdir()
                project_config = project_seaforge / "allowed_commands.yaml"
