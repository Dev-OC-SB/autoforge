...
This document defines the mandatory project vision for SeaForge. All contributions must align with these principles. PRs that deviate from this vision will be rejected. This file itself is immutable via PR — any PR that modifies VISION.md will be rejected outright.

## Claude Agent SDK Exclusivity

SeaForge is a wrapper around the **Claude Agent SDK**. This is a foundational architectural decision, not a preference.

**What this means:**

- SeaForge only supports providers, models, and integrations that work through the Claude Agent SDK.
...
- PRs introducing abstractions designed to make SeaForge "agent-agnostic" will be rejected.
