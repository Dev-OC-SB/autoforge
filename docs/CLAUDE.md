...
seaforge                    # Start server (first run sets up Python venv)
seaforge config             # Edit ~/.seaforge/.env in $EDITOR
seaforge config --show      # Print active configuration
seaforge --port 9999        # Custom port
seaforge --no-browser       # Don't auto-open browser
seaforge --repair           # Delete and recreate ~/.seaforge/venv/
```
...
The `seaforge` command is a Node.js wrapper that manages the Python environment and server lifecycle:
- `bin/seaforge.js` - Entry point (shebang script)
- `lib/cli.js` - Main CLI logic: Python 3.11+ detection (cross-platform), venv management at `~/.seaforge/venv/` with composite marker (requirements hash + Python version), `.env` config loading from `~/.seaforge/.env`, uvicorn server startup with PID file, and signal handling
- `package.json` - npm package config (`seaforge-ai` on npm), `files` whitelist with `__pycache__` exclusions, `prepublishOnly` builds the UI
...
- `seaforge_paths.py` - Central path resolution with dual-path backward compatibility and migration
...
- **All platforms**: `~/.seaforge/registry.db`
...
Projects can be stored in any directory (registered in `~/.seaforge/registry.db`). Each project contains:
- `.seaforge/prompts/app_spec.txt` - Application specification (XML format)
- `.seaforge/prompts/initializer_prompt.md` - First session prompt
- `.seaforge/prompts/coding_prompt.md` - Continuation session prompt
- `.seaforge/features.db` - SQLite database with feature test cases
- `.seaforge/.agent.lock` - Lock file to prevent multiple agent instances
- `.seaforge/allowed_commands.yaml` - Project-specific bash command allowlist (optional)
- `.seaforge/.gitignore` - Ignores runtime files
...
Legacy projects with files at root level (e.g., `features.db`, `prompts/`) are auto-migrated to `.seaforge/` on next agent start. Dual-path resolution ensures old and new layouts work transparently.
...
2. **Org Blocklist** (`~/.seaforge/config.yaml`) - Cannot be overridden by projects
3. **Org Allowlist** (`~/.seaforge/config.yaml`) - Available to all projects
4. **Global Allowlist** (`security.py`) - Default commands (npm, git, curl, etc.)
5. **Project Allowlist** (`.seaforge/allowed_commands.yaml`) - Project-specific commands

**Project Configuration:**

Each project can define custom allowed commands in `.seaforge/allowed_commands.yaml`:
...
System administrators can set org-wide policies in `~/.seaforge/config.yaml`:
...
- `/gsd-to-seaforge-spec` - Convert GSD codebase mapping to app_spec.txt
...
- `gsd-to-seaforge-spec` - Convert GSD codebase mapping to SeaForge app_spec format
...
1. Project-specific: `{project_dir}/.seaforge/prompts/{name}.md` (or legacy `{project_dir}/prompts/{name}.md`)
2. Base template: `.claude/templates/{name}.template.md`

### Agent Session Flow

1. Check if `.seaforge/features.db` has features (determines initializer vs coding agent)
