...
from core.registry import DEFAULT_MODEL, get_effective_sdk_env, get_project_path
...
def main() -> None:
    ...
    # Migrate project layout to .seaforge/ if needed (idempotent, safe)
    from core.autoforge_paths import migrate_project_layout
    migrated = migrate_project_layout(project_dir)
    if migrated:
        print(f"Migrated project files to .seaforge/: {', '.join(migrated)}", flush=True)

    # Migrate project to current SeaForge version (idempotent, safe)
    from core.prompts import migrate_project_to_current
    ...
    try:
        if args.agent_type:
            ...
            from core.parallel_orchestrator import run_parallel_orchestrator
