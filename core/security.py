...
def get_org_config_path() -> Path:
    ...
    Returns:
        Path to ~/.seaforge/config.yaml (falls back to ~/.autocoder/config.yaml)
    """
    new_path = Path.home() / ".seaforge" / "config.yaml"
    if new_path.exists():
        return new_path
...
def load_org_config() -> Optional[dict]:
    """
    Load organization-level config from ~/.seaforge/config.yaml.
...
def load_project_commands(project_dir: Path) -> Optional[dict]:
    ...
    config_path = project_dir.resolve() / ".seaforge" / "allowed_commands.yaml"
    if not config_path.exists():
...
async def bash_security_hook(input_data, tool_use_id=None, context=None):
    ...
    for cmd in commands:
        ...
        if not is_command_allowed(cmd, allowed_commands):
            ...
            error_msg += "  1. Add to .seaforge/allowed_commands.yaml for this project, OR\n"
