...
from core.security import SENSITIVE_DIRECTORIES, bash_security_hook
...
async def create_unified_client(
    project_dir: Path,
    model: str,
    yolo_mode: bool = False,
    agent_type: str = "coding",
    ...
    """
    Create a unified client that supports multiple LLM providers.
    
    This is the new multi-provider entry point that can use:
    - Ollama (local, free)
    - OpenRouter (cloud, multi-model)
    - OpenAI (cloud)
    - Claude CLI (legacy, requires subscription)
    
    Args:
        project_dir: Directory for the project
        model: Model to use (provider-specific)
        yolo_mode: If True, skip browser testing
        agent_type: One of "coding", "testing", or "initializer"
        provider_override: Optional explicit provider selection
        
    Returns:
        UnifiedClient instance wrapping the appropriate backend
        
    Raises:
        RuntimeError: If no provider is available
    """
    from core.unified_client import UnifiedClient, detect_available_provider, get_provider_model
    ...
    if provider_override:
        ...
        reason = f"Explicitly specified: {provider}"
    else:
    ...
    provider_model = get_provider_model(provider, config, model)
    
    # Create backend based on provider
    if provider == "claude_cli":
        # Use existing Claude SDK client
        print(f"   Using Claude SDK with model: {provider_model}")
        backend = create_client(project_dir, provider_model, yolo_mode, agent_type)
        return UnifiedClient(backend, "claude_cli", provider_model)
    
    else:
        # Use adapter-based client
        print(f"   Using {provider} adapter with model: {provider_model}")
        
        # Create adapter client
        adapter_client = AdapterClient(config)
        
        # Wire up tool executor for agentic loop
        from core.tool_executor import ToolExecutor
        tool_executor = ToolExecutor(project_dir)
        if hasattr(adapter_client.adapter, 'set_tool_executor'):
            adapter_client.adapter.set_tool_executor(tool_executor)
            print(f"   ✓ Agentic tool-use loop enabled")
        
        # Test connection
        test_result = await adapter_client.test_connection()
        if not test_result["ok"]:
            raise RuntimeError(
                f"Provider '{provider}' not available: {test_result['message']}\n"
                f"Please check your configuration and try again."
            )
        
        print(f"   ✓ Connection test passed")
        
        return UnifiedClient(adapter_client, provider, provider_model)
