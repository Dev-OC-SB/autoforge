...
Uses SQLite database stored at ~/.seaforge/registry.db.
"""
...
def _migrate_registry_dir() -> None:
    """Migrate ~/.autocoder/ to ~/.seaforge/ if needed.
    ...
    new_dir = Path.home() / ".seaforge"
    if old_dir.exists() and not new_dir.exists():
        try:
            old_dir.rename(new_dir)
            logger.info("Migrated registry directory: ~/.autocoder/ -> ~/.seaforge/")
        except Exception:
            logger.warning("Failed to migrate ~/.autocoder/ to ~/.seaforge/", exc_info=True)
...
LEGACY_MODEL_MAP = {
    "claude-opus-4-5-20251101": "minimax/minimax-m2.5",
    "claude-opus-4-6": "minimax/minimax-m2.5",
    "claude-sonnet-4-5-20250929": "minimax/minimax-m2.5",
...
def get_config_dir() -> Path:
    """
    Get the config directory: ~/.seaforge/

    Automatically migrates from ~/.autocoder/ if needed.

    Returns:
        Path to ~/.seaforge/ (created if it doesn't exist)
    """
    _migrate_registry_dir()
    config_dir = Path.home() / ".seaforge"
...
def get_project_by_path(path: Path) -> Project | None:
    """
    ...
    try:
        ...
        project = session.query(Project).filter(Project.path == path_str).first()
        if project is None:
...
def get_effective_sdk_env() -> dict[str, str]:
    ...
    if provider_id == "claude":
        ...
        from core.env_constants import API_ENV_VARS
