...
from core.progress import has_features
from server.utils.process_utils import kill_process_tree

logger = logging.getLogger(__name__)

# Root directory of seaforge (where this script and autonomous_agent_demo.py live)
SEAFORGE_ROOT = Path(__file__).parent.resolve()

# Debug log file path
DEBUG_LOG_FILE = SEAFORGE_ROOT / "orchestrator_debug.log"
...
class ParallelOrchestrator:
    ...
    def __init__(
        ...
        self._task_assignment_lock = threading.Lock()
        self._github_config = None
        self._external_agents = []
        self._gitbot_sync = None
        self._agent_zero_clients = {}
        self._error_tracker = None
        self._task_assignments: dict[int, dict] = {}  # feature_id -> task_assignment
    ...
    def _load_github_config(self):
        ...
        from api.database import GitHubConfig
        from server.services.gitbot_style_sync import GitbotStyleSync
        from server.services.encryption import decrypt_token
        from server.services.error_tracker import ErrorTracker
        
        session = self.get_session()
        try:
            # Load GitHub config - just get the first one for this project
            # Since we're running from a specific project directory, there should only be one config
            github_config = session.query(GitHubConfig).first()
            
            if not github_config:
                logger.warning(f"No GitHub configuration found for project")
                logger.warning("GitHub configuration is mandatory. Please configure via UI or API.")
                return
            
            self._github_config = github_config
            
            # Get project name from path for temp directory
            project_name = self.project_dir.name
            
            # Initialize GitbotStyleSync
            from core.autoforge_paths import ensure_temp_project_dir
            temp_dir = ensure_temp_project_dir(project_name)
            
            self._gitbot_sync = GitbotStyleSync(
                project_dir=temp_dir,
                repo_url=github_config.repo_url,
                auth_token=decrypt_token(github_config.auth_token),
                task_code=github_config.task_code
            )
            
            # Initialize error tracker
            self._error_tracker = ErrorTracker(session)
            
            logger.info(f"Loaded GitHub config for {github_config.task_code}")
            print(f"✓ GitHub integration enabled: {github_config.task_code}")
    ...
    def _load_external_agents(self):
        """Load external agent configurations from database."""
        from api.database import ExternalAgent
        from server.services.agent_zero_a2a_client import AgentZeroA2AClient
        from server.services.encryption import decrypt_token
        
        session = self.get_session()
        try:
            # Load external agents for this project
            # Since we're running from a specific project directory, load all enabled external agents
            external_agents = session.query(ExternalAgent).filter(
                ExternalAgent.enabled == True
            ).all()
            
            self._external_agents = external_agents
            
            # Create A2A clients
            for agent in external_agents:
                try:
                    client = AgentZeroA2AClient(
                        agent_name=agent.name,
                        base_url=agent.base_url,
                        api_token=decrypt_token(agent.api_token)
                    )
                    self._agent_zero_clients[agent.id] = client
                    logger.info(f"Loaded external agent: {agent.name} at {agent.base_url}")
                except Exception as e:
                    logger.error(f"Failed to create client for {agent.name}: {e}")
            
        except Exception as e:
            logger.error(f"Failed to load external agents: {e}")
        finally:
            session.close()
    
    def _get_next_task_assignment(self) -> dict:
        """
        Get next task assignment with unique ID and branch name.
        Thread-safe task ID assignment.
        
        Checks for existing branches on remote to ensure sequential task assignment
        and avoid gaps that would cause waiting for non-existent branches.
        
        Returns:
            dict with task_id, branch_name, previous_branch, task_type
        """
        ...
        with self._task_assignment_lock:
            ...
            try:
                ...
                # Increment task ID
                task_id = github_config.current_task_id + 1
                
                # Generate branch name
                date_str = datetime.now().strftime("%d/%m/%y")
                branch_name = f"{github_config.task_code}-{task_id:03d}-{date_str}"
                
                # Get previous branch - check if it exists on remote
                previous_branch = github_config.last_branch or "main"
                
                # If previous_branch is not main, verify it exists on remote
                if previous_branch != "main" and self._gitbot_sync:
                    try:
                        result = subprocess.run(
                            ["git", "ls-remote", "--heads", "origin", previous_branch],
                            cwd=self._gitbot_sync.project_dir,
                            capture_output=True,
                            text=True,
                            check=False,
                            timeout=10
                        ...
                        if not result.stdout.strip():
                            # Previous branch doesn't exist on remote, use main instead
                            print(f"[GIT] Previous branch {previous_branch} not on remote, using main instead")
                            logger.warning(f"Previous branch {previous_branch} not on remote, using main instead")
                            previous_branch = "main"
                    except Exception as e:
                        logger.warning(f"Error checking previous branch: {e}, using main")
                        previous_branch = "main"
                
                # Update task ID and last branch
                github_config.current_task_id = task_id
                github_config.last_branch = branch_name
                
                # Determine task type
                task_type = "planning" if task_id < 100 else "development"
                
                session.commit()
                
                logger.info(f"Assigned task {github_config.task_code}-{task_id:03d} ({task_type})")
    ...
    def _wait_for_branch_on_remote(self, branch_name: str, max_wait: int = 120, poll_interval: int = 5) -> bool:
        """
        Wait for a branch to appear on the remote.
        
        Args:
            branch_name: Branch name to wait for
            max_wait: Maximum seconds to wait
            poll_interval: Seconds between checks
            
        Returns:
            True if branch found, False if timed out
        """
        import subprocess
        import time
        ...
        while waited < max_wait:
            try:
                ...
                if result.stdout.strip():
                    print(f"[GIT] Previous branch {branch_name} found on remote")
                    return True
            except Exception as e:
                logger.warning(f"Error checking remote for {branch_name}: {e}")
            
            print(f"[GIT] Waiting for {branch_name} on remote... ({waited}s/{max_wait}s)")
            time.sleep(poll_interval)
            waited += poll_interval
        
        return False

    def _setup_git_branch(self, task_assignment: dict):
        """
        Setup git branch before agent runs.
        
        The previous_branch in task_assignment is already validated by
        _get_next_task_assignment() — if it wasn't on the remote, it was
        set to 'main'. So here we only need to wait if previous_branch
        is a real task branch (not 'main'), meaning another parallel agent
        is currently pushing it.
        
        Args:
            task_assignment: Task assignment dict with branch_name, previous_branch
        """
        if not self._gitbot_sync:
            return
        
        branch_name = task_assignment["branch_name"]
        previous_branch = task_assignment["previous_branch"]
        
        try:
            # Ensure git initialized and main branch exists
            self._gitbot_sync.ensure_git_initialized()
            self._gitbot_sync.ensure_main_branch_exists()
            
            # If previous_branch is a task branch (not main), wait for it.
            # _get_next_task_assignment already validated it exists OR set it to main,
            # so if it's not main here, it means another parallel agent is pushing it
            # right now — we should wait briefly.
            cherry_pick_branch = None
            if previous_branch != "main":
                if self._wait_for_branch_on_remote(previous_branch, max_wait=120, poll_interval=5):
                    cherry_pick_branch = previous_branch
                else:
                    print(f"[GIT WARNING] Timeout waiting for {previous_branch}, skipping cherry-pick")
                    logger.warning(f"Timeout waiting for {previous_branch}, skipping cherry-pick")
            
            # Create branch from main
            self._gitbot_sync.create_branch_from_main(branch_name)
            
            # Cherry-pick from previous branch if available
            if cherry_pick_branch:
                try:
                    self._gitbot_sync.cherry_pick_from_previous(cherry_pick_branch)
                except Exception as e:
                    print(f"[GIT WARNING] Cherry-pick from {cherry_pick_branch} failed: {e}")
                    logger.warning(f"Cherry-pick from {cherry_pick_branch} failed: {e}")
            
            print(f"[GIT] Branch setup complete: {branch_name} (from {previous_branch})")
            logger.info(f"Git branch setup complete: {branch_name}")
            
        except Exception as e:
            print(f"[GIT ERROR] Branch setup failed for {branch_name}: {e}")
            logger.error(f"Git branch setup failed: {e}")
            try:
                self._gitbot_sync.create_branch_from_main(branch_name)
                print(f"[GIT] Branch setup retry succeeded (without cherry-pick)")
            except Exception as e2:
                print(f"[GIT ERROR] Branch setup retry also failed: {e2}")
                logger.error(f"Git branch setup retry failed: {e2}")
                raise
    
    def _sync_project_to_git_dir(self):
        """
        Sync project files from the actual project directory to the temp git directory.
        
        The agent writes code in self.project_dir but git operations happen in the
        temp git directory (self._gitbot_sync.project_dir). We need to copy changes
        from project dir to git dir before committing.
        """
        import shutil
        
        src = self.project_dir
        dst = self._gitbot_sync.project_dir
        
        if src == dst:
            return  # Same directory, no sync needed
        
        print(f"[GIT] Syncing project files: {src} -> {dst}")
        
        # Copy all files from project dir to git dir, excluding .git and .seaforge
        exclude_dirs = {'.git', '.seaforge', '.autoforge', 'node_modules', '__pycache__', '.venv', 'venv'}
        exclude_files = {'.gitignore'}  # We'll handle .gitignore separately if needed
        ...
        print(f"[GIT] Project files synced successfully")

    def _commit_and_push(self, task_assignment: dict, feature: Feature):
        """
        Commit and push changes after agent completes.
        
        Args:
            task_assignment: Task assignment dict
        ...
        try:
            # Sync project files to git directory before committing
            self._sync_project_to_git_dir()
            
            # Commit changes
            commit_message = f"feat({task_assignment['task_id']:03d}): {feature.name}"
            commit_hash = self._gitbot_sync.commit_changes(commit_message)
            
            if not commit_hash:
                print(f"[GIT] No changes to commit for feature {feature.id}")
                logger.info("No changes to commit")
                return
            
            print(f"[GIT] Committed: {commit_hash[:8]} - {commit_message}")
            logger.info(f"Committed changes: {commit_hash}")
            
            # Push to GitHub
            print(f"[GIT] Pushing branch {task_assignment['branch_name']} to remote...")
            self._gitbot_sync.push_branch(task_assignment["branch_name"])
            print(f"[GIT] ✓ Pushed branch {task_assignment['branch_name']} to remote")
            logger.info(f"Pushed branch: {task_assignment['branch_name']}")
            
            # Create PR if development phase (task_id >= 100)
            if task_assignment["task_id"] >= 100:
                import asyncio
                pr_title = commit_message
                pr_body = f"Automated PR for task {task_assignment['task_code']}-{task_assignment['task_id']:03d}\n\n{feature.description or ''}"
                
                # Run async PR creation in sync context
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                try:
                    pr_url = loop.run_until_complete(
                        self._gitbot_sync.create_pull_request(
                            branch_name=task_assignment["branch_name"],
                            title=pr_title,
                            body=pr_body
                        )
            ...
            # Retry once
            try:
                logger.info("Retrying git push...")
                self._gitbot_sync.push_branch(task_assignment["branch_name"])
                logger.info("Git push retry succeeded")
            except Exception as e2:
                logger.error(f"Git push retry failed: {e2}")
                # Log error and create GitHub issue
                if self._error_tracker:
                    self._log_git_error("git_push", str(e2), {
                        "feature_id": feature.id,
                        "branch": task_assignment["branch_name"],
                        "task_id": task_assignment["task_id"]
                    })
                raise
    
    def _log_git_error(self, error_type: str, error_message: str, context: dict):
        """
        Log git error and create GitHub issue.
        ...
        """
        if not self._error_tracker or not self._github_config:
            return
        
        try:
            # Get project ID
            from core.registry import get_project_by_path
            project = get_project_by_path(self.project_dir)
            if not project:
                return
            
            # Log error
            error_log = self._error_tracker.log_error(
            ...
            # Create GitHub issue
            from server.services.github_issue_creator import GitHubIssueCreator
            from server.services.encryption import decrypt_token
            
            issue_creator = GitHubIssueCreator(
                repo_url=self._github_config.repo_url,
                auth_token=decrypt_token(self._github_config.auth_token)
            )
            
            title = f"SeaForge Git Error: {error_type}"
            body = GitHubIssueCreator.format_error_body(
                error_message=error_message,
                error_type=error_type,
                context=context,
                retry_count=1
            ...
            # Run async issue creation in sync context
            import asyncio
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                issue_url = loop.run_until_complete(
                    issue_creator.create_issue_if_not_exists(
                        title=title,
                        body=body,
                        labels=["seaforge-error", "git-operation-failed"]
                    )
                )
                
                if issue_url:
                    self._error_tracker.update_issue_url(error_log.id, issue_url)
                    logger.info(f"Created GitHub issue for error: {issue_url}")
            finally:
                loop.close()
                
        except Exception as e:
            logger.error(f"Failed to log git error: {e}")
    ...
    def _spawn_coding_agent(self, feature_id: int) -> tuple[bool, str]:
        ...
        # Get task assignment if GitHub config exists
        task_assignment = None
        print(f"[GIT DEBUG] _spawn_coding_agent called for feature {feature_id}")
        print(f"[GIT DEBUG] _gitbot_sync exists: {self._gitbot_sync is not None}")
        
        if self._gitbot_sync:
            try:
                print(f"[GIT DEBUG] Getting task assignment for feature {feature_id}")
                task_assignment = self._get_next_task_assignment()
                print(f"[GIT DEBUG] Task assignment created: {task_assignment['branch_name']}")
                logger.info(f"Task assignment for feature {feature_id}: {task_assignment['branch_name']}")
                
                # Store task assignment for later retrieval
                self._task_assignments[feature_id] = task_assignment
                print(f"[GIT DEBUG] Stored task assignment for feature {feature_id}")
                
                # Setup git branch before agent runs
                print(f"[GIT DEBUG] Setting up git branch: {task_assignment['branch_name']}")
                self._setup_git_branch(task_assignment)
                print(f"[GIT DEBUG] Git branch setup complete")
            except Exception as e:
                print(f"[GIT ERROR] Failed to setup git for feature {feature_id}: {e}")
                logger.error(f"Failed to setup git for feature {feature_id}: {e}")
                if self._error_tracker:
                    self._log_git_error("git_setup", str(e), {"feature_id": feature_id})
                # Continue without git workflow if setup fails
        else:
            print(f"[GIT DEBUG] _gitbot_sync is None, skipping git workflow")
        ...
        cmd = [
            ...
            str(SEAFORGE_ROOT / "autonomous_agent_demo.py"),
        ...
        try:
            # Build environment with task assignment info
            env = {
                **os.environ,
                "PYTHONUNBUFFERED": "1",
                "NODE_COMPILE_CACHE": "",
                "PLAYWRIGHT_CLI_SESSION": f"coding-{feature_id}"
            }
            
            # Add git workflow environment variables
            if task_assignment:
                env.update({
                    "SEAFORGE_TASK_ID": str(task_assignment["task_id"]),
                    "SEAFORGE_TASK_CODE": task_assignment["task_code"],
                    "SEAFORGE_BRANCH_NAME": task_assignment["branch_name"],
                    "SEAFORGE_PREVIOUS_BRANCH": task_assignment["previous_branch"],
                    "SEAFORGE_TASK_TYPE": task_assignment["task_type"]
                })
    ...
    def _spawn_coding_agent_batch(self, feature_ids: list[int]) -> tuple[bool, str]:
        ...
        # Get task assignment if GitHub config exists
        task_assignment = None
        print(f"[GIT DEBUG] _spawn_coding_agent_batch called for features {feature_ids}")
        print(f"[GIT DEBUG] _gitbot_sync exists: {self._gitbot_sync is not None}")
        
        if self._gitbot_sync:
            try:
                print(f"[GIT DEBUG] Getting task assignment for batch primary {primary_id}")
                task_assignment = self._get_next_task_assignment()
                print(f"[GIT DEBUG] Task assignment created: {task_assignment['branch_name']}")
                logger.info(f"Task assignment for batch {primary_id}: {task_assignment['branch_name']}")
                
                # Store task assignment for the primary feature
                self._task_assignments[primary_id] = task_assignment
                print(f"[GIT DEBUG] Stored task assignment for primary {primary_id}")
                ...
                print(f"[GIT ERROR] Failed to setup git for batch {primary_id}: {e}")
                logger.error(f"Failed to setup git for batch {primary_id}: {e}")
                if self._error_tracker:
                    self._log_git_error("git_setup", str(e), {"feature_ids": feature_ids})
                # Continue without git workflow if setup fails
        else:
            print(f"[GIT DEBUG] _gitbot_sync is None, skipping git workflow")
        ...
        try:
            ...
            env = {
                **os.environ,
                "PYTHONUNBUFFERED": "1",
                "NODE_COMPILE_CACHE": "",
                "PLAYWRIGHT_CLI_SESSION": f"coding-{primary_id}"
            }
    ...
    def _run_inter_session_cleanup(self):
        ...
        """
        try:
            from core.temp_cleanup import cleanup_project_screenshots, cleanup_stale_temp
            cleanup_stale_temp()
    ...
    def _on_agent_complete(
        ...
        try:
            ...
            # Post-agent git operations (commit and push)
            if return_code == 0:
                print(f"[GIT DEBUG] Agent completed successfully for feature {feature_id}")
                print(f"[GIT DEBUG] _gitbot_sync is: {self._gitbot_sync is not None}")
                print(f"[GIT DEBUG] Task assignments: {list(self._task_assignments.keys())}")
                
                if self._gitbot_sync:
                    # Get task assignment for the primary feature
                    task_assignment = self._task_assignments.get(feature_id)
                    print(f"[GIT DEBUG] Task assignment for {feature_id}: {task_assignment is not None}")
                    
                    if task_assignment:
                        # Get the primary feature for commit message
                        primary_feature = session.query(Feature).filter(Feature.id == feature_id).first()
                        if primary_feature:
                            try:
                                print(f"[GIT] Performing git operations for feature {feature_id}")
                                logger.info(f"Performing git operations for feature {feature_id}")
                                self._commit_and_push(task_assignment, primary_feature)
                                print(f"[GIT] Git operations completed for feature {feature_id}")
                                logger.info(f"Git operations completed for feature {feature_id}")
                            except Exception as e:
                                print(f"[GIT ERROR] Git operations failed for feature {feature_id}: {e}")
                                logger.error(f"Git operations failed for feature {feature_id}: {e}")
                                # Error already logged in _commit_and_push
                        
                        # Clean up task assignment
                        self._task_assignments.pop(feature_id, None)
                else:
                    print(f"[GIT DEBUG] Git workflow not initialized - skipping git operations")
        finally:
            session.close()
    ...
    async def run_loop(self):
        ...
        # Load GitHub configuration and external agents
...
def main():
    ...
    from core.registry import DEFAULT_MODEL, get_project_path
