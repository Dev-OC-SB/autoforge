...
def scaffold_project_prompts(project_dir: Path) -> Path:
    ...
    # Create .seaforge directory with .gitignore for runtime files
    from core.autoforge_paths import ensure_seaforge_dir
    ...
    # Copy allowed_commands.yaml template to .seaforge/
    examples_dir = Path(__file__).parent / "examples"
    allowed_commands_template = examples_dir / "project_allowed_commands.yaml"
    allowed_commands_dest = seaforge_dir / "allowed_commands.yaml"
    if allowed_commands_template.exists() and not allowed_commands_dest.exists():
        try:
            ...
            copied_files.append(".seaforge/allowed_commands.yaml")
        except (OSError, PermissionError) as e:
...
def _get_migration_version(project_dir: Path) -> int:
    """Read the migration version from .seaforge/.migration_version."""
    from core.autoforge_paths import get_seaforge_dir
    version_file = get_seaforge_dir(project_dir) / ".migration_version"
...
def _set_migration_version(project_dir: Path, version: int) -> None:
    """Write the migration version to .seaforge/.migration_version."""
    from core.autoforge_paths import get_seaforge_dir
    version_file = get_seaforge_dir(project_dir) / ".migration_version"
...
def migrate_project_to_current(project_dir: Path) -> list[str]:
    """Migrate an existing project to the current SeaForge version.
