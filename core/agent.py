...
from core.client import create_client, create_unified_client
...
async def run_agent_session(
    client,  # UnifiedClient or ClaudeSDKClient
    message: str,
    project_dir: Path,
) -> tuple[str, str]:
    """
    Run a single agent session using Claude Agent SDK or adapter-based client.

    Args:
        client: UnifiedClient or ClaudeSDKClient instance
...
async def run_autonomous_agent(
    ...
    while True:
        ...
        # Create client (fresh context) - use unified client for multi-provider support
        client = await create_unified_client(project_dir, model, yolo_mode=yolo_mode, agent_type=agent_type)
